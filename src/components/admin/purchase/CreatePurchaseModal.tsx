...
export function CreatePurchaseModal({ isOpen, onClose }: CreatePurchaseModalProps) {
        ...
        <form onSubmit={handleSubmit} className="space-y-6 mt-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input id="provider" placeholder="Nhập tên nhà cung cấp" required />
            </div>
            <div className="space-y-2">
          ...
          <div className="border rounded-lg p-4 bg-gray-50">
              ...
              {items.map((item, index) => (
                <div key={index} className="flex items-center gap-3 bg-white p-3 rounded-md border">
                  <div className="flex-1">
                    <Label className="text-xs text-gray-500 mb-1">Tên sách</Label>
                    <select 
                      className="w-full border border-gray-300 rounded-md p-2 text-sm outline-none"
                      value={item.bookId}
                      onChange={(e) => handleChange(index, "bookId", e.target.value)}
                      required
                    >
                  ...
                  <div className="w-32">
                    <Label className="text-xs text-gray-500 mb-1">Giá nhập</Label>
                    <Input 
                      type="number" 
                      min="0" 
                      value={item.price}
                  ...
                  <div className="w-32 text-right">
                    <Label className="text-xs text-gray-500 mb-1">Thành tiền</Label>
            ...
            <div className="mt-4 pt-4 border-t flex justify-end">
              <div className="text-right">
                ...
                <p className="text-xl font-bold text-blue-600">{totalAmount.toLocaleString("vi-VN")} đ</p>
              </div>
            </div>
          </div>
