...
export default function AccountsPage() {
  ...
  const toggleStatus = (id: number, currentStatus: string) => {
    ...
    updateStatus.mutate({ id, status: newStatus }, {
      onSuccess: () => {
        toast.success(`Đã ${newStatus === "BLOCKED" ? "khóa" : "mở khóa"} tài khoản`);
      }
    });
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4 bg-white p-4 rounded-lg shadow-sm border">
        <div className="relative flex-1 max-w-md">
          ...
          <Input className="pl-10" placeholder="Tìm kiếm theo email hoặc tên..." />
        </div>
      </div>
                ...
                <TableCell className="text-right">
                  {user.role !== 'ADMIN' && (
                    <Button 
                      ...
                      className={(user.status || "ACTIVE") === 'ACTIVE' ? 'text-red-600 hover:text-red-700 hover:bg-red-50' : 'text-green-600 hover:text-green-700 hover:bg-green-50'}
                    >
                      {(user.status || "ACTIVE") === 'ACTIVE' ? <><Lock className="w-4 h-4 mr-1" /> Khóa</> : <><Unlock className="w-4 h-4 mr-1" /> Mở khóa</>}
                    </Button>
                  )}
                  {user.role === 'ADMIN' && (
